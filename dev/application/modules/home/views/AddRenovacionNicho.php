<script type="text/javascript">
        ...
        if (lado > 0) {
            ...
            $.post("<?php echo base_url() . "home/getNichosRenovar"; ?>", datos, function (nichos) {
                ...
                $.each(nn, function (index, v) {
                    $comboNichoLibres.append("<option data-fecha=" + v['transcurrido'] + " data-edad=" + v['edadFallecido'] + " value=" + v['id_nicho'] + ">" + v['nicho'] + "</option>");
                });
            }, 'json');
        }
        else
    ...
    $('#nichoLibres').change(function () {
        ...
        $('#tiempoTrascurrido').val(fecha);
 
        $('#transcurido').text(fecha+" años Transcurridos");
    ...
    $('#generarCosto').click(function () {
        ...
        if (tipo == "Mayor") {
            ...
            if (clase == "Antiguo") {
                $('#costo').val(80 * parseInt(fecha));
            }
        }
        if (tipo == "Menor") {
            if (clase == "Nuevo") {
                $('#costo').val(100 * parseInt(fecha));
            }
            if (clase == "Antiguo") {
                $('#costo').val(60 * parseInt(fecha));
            }
        }
    });
...
<?php echo $bloque_info[0]['nombre']; ?>
    ...
    <div class="control-group">
        ...
        <div class="controls">
            ...
            <div id="verT" style="display: none;">El costo que genero ya fue multiplicado por el tiempo transcurrido</div>
            <input type="hidden" id="tiempoTrascurrido" name="tiempoTrascurrido">
        </div>
    </div>
